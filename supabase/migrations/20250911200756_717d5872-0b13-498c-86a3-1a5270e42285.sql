...
-- press_releases: add version_number and extend status enum
...
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
  ALTER TYPE public.press_release_status ADD VALUE IF NOT EXISTS 'approved';
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
  ALTER TYPE public.press_release_status ADD VALUE IF NOT EXISTS 'published';
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
-- RLS: Admins can manage approval history
DO $$ BEGIN
  CREATE POLICY IF NOT EXISTS "Admins manage approval history" ON public.approval_history
  FOR ALL TO authenticated
  USING (public.has_role(auth.uid()::text, 'admin'))
  WITH CHECK (public.has_role(auth.uid()::text, 'admin'));
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- RLS: Users can view their own approval actions
DO $$ BEGIN
  CREATE POLICY IF NOT EXISTS "Users view own approvals" ON public.approval_history
